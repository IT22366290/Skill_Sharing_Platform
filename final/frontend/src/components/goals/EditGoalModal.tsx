...
import { LearningPlan, Topic } from '../../types/learningPlan';
interface EditGoalModalProps {
  goal: LearningPlan;
  onClose: () => void;
  onUpdate: (goal: LearningPlan) => void;
}
const EditGoalModal: React.FC<EditGoalModalProps> = ({
  ...
  const [category, setCategory] = useState(goal.categoryName);
  const [startDate, setStartDate] = useState(goal.startDate.split('T')[0]);
  const [targetDate, setTargetDate] = useState(goal.endDate.split('T')[0]);
  const [status, setStatus] = useState(goal.status);
  const [topics, setTopics] = useState<Topic[]>(goal.topics);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedGoal: LearningPlan = {
        ...goal,
        title,
        description,
  ...
  const toggleTopic = (index: number) => {
    setTopics(topics.map((t, i) => i === index ? {
      ...t,
      status: t.status === 'Finished' ? 'NotFinished' : 'Finished'
    } : t));
  };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={status} onChange={e => setStatus(e.target.value as LearningPlan['status'])} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500">
              ...
              <div className="mt-2 space-y-2">
                {topics.map((topic, index) => <div key={index} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded-lg">
                    <div className="flex items-center flex-1">
                      <button type="button" onClick={() => toggleTopic(index)} className={`p-1 rounded-full mr-2 ${topic.status === 'Finished' ? 'text-green-500' : 'text-gray-400'}`}>
                        <CheckIcon size={16} />
                      </button>
                      <span className={`text-sm ${topic.status === 'Finished' ? 'line-through text-gray-500' : 'text-gray-700'}`}>
                        {topic.topic}
                      </span>
                    </div>
